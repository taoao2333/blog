...
class Captcha{
	...
	public function generate(){
		...
		for($i=1;$i<=$this->stringnum;$i++){
			$stringcolor = imagecolorallocate($img, mt_rand(0,255), mt_rand(0,100), mt_rand(0,100));
			imagestring($img, 5, $i*$span, ($this->height/2)-8, $this->string[$i-1], $stringcolor);
		}
		//5，添加干扰线
		for($i=1;$i<=$this->linenum;$i++){
			$linecolor = imagecolorallocate($img, mt_rand(0,255), mt_rand(30,255), mt_rand(200,255));
